...
export function DetailPanel({ person, unitName, canEdit, onTool, onToast, onSaved }: {
  person: Person;
  unitName: string;
  canEdit: boolean;
  onTool: (a: "query" | Employ) => void;
  onToast: (t: "success" | "error" | "info", m: string) => void;
  onSaved?: () => void; // 需求2：保存后通知父级刷新左侧最新职务
}) {
  ...
  const [deductOpen, setDeductOpen] = useState(false); // 需求3：考核扣除年限弹层
  const [taogaiOpen, setTaogaiOpen] = useState(false); // 需求7：套改明细弹窗
  ...
  /* 需求2/4：最新职务（优先取测算结果，其次取职务变化最新一条） */
    ...
    <div className="anim-panel flex-1 min-h-0 flex flex-col gap-3">
      ...
      <div className="flex-1 min-h-0 flex gap-3">
        ...
        <div className="flex-1 min-w-0 flex flex-col gap-3">
                ...
                <div className="border-t border-dashed border-[var(--line)] pt-2.5">
                  ...
                  <div className="grid grid-cols-[88px_minmax(0,1fr)_30px] items-end gap-1.5">
                    ...
                    <button
                      ...
                      className={`h-8 rounded-md border flex items-center justify-center transition active:scale-95 ${
                        deductOpen
                          ? "border-[rgba(10,132,255,.6)] bg-[var(--sel)] text-[var(--acc)]"
                          : "border-[var(--line)] bg-[var(--bg-3)] text-[var(--tx-2)] hover:text-[var(--acc)] hover:border-[rgba(10,132,255,.5)]"
                      }`}
                    >
                  ...
                  {deductOpen && (
                    <div className="mt-1.5 anim-fade flex items-center gap-2 rounded-md border border-[rgba(10,132,255,.4)] bg-[var(--sel)] px-2.5 py-2">
                      ...
                      <button onClick={() => setDeductOpen(false)} className="ml-auto text-[var(--tx-3)] hover:text-[var(--tx-1)] transition" title="收起">
          ...
          {/* 下部：工资演变明细（需求9，收窄；需求7：套改明细按钮置于其后） */}
        ...
        <SalaryPanel
          personId={person.id}
          results={results}
          latestDutyIndex={latestDutyIndex}
          canEdit={canEdit}
          onToast={onToast}
        />
      </div>
      ...
      {/* 需求7：套改明细弹窗 */}
      {taogaiOpen && (
        <TaogaiModal person={person} results={results} onClose={() => setTaogaiOpen(false)} />
      )}
    </div>
  );
}
