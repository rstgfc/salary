...
/* 业务预测：筛选 / 排序共用控件（条件与行为与左侧名单完全一致；结果经 onView 上报） */
export function PersonFilterSort({ persons, units, onView }: {
  persons: Person[];
  units: Unit[];
  onView: (v: Person[]) => void;
}) {
  ...
  useEffect(() => { onView(view); }, [view]); // eslint-disable-line react-hooks/exhaustive-deps
...
export function PersonList({ persons, total, selectedId, onSelect, query, onQuery, units, tick, onQueryModal }: {
  ...
  /* 需求2：筛选 + 排序（共用控件 PersonFilterSort，结果回调） */
