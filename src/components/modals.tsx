...
/* ================= 单位增加 ================= */
export function UnitModal({ units, persons, onClose, onAdd, onRemove, onEdit }: {
            ...
            <div key={u.id} className="flex items-center gap-2.5 px-3 py-2 hover:bg-[var(--hov)] transition">
              ...
              {editId === u.id ? (
                ...
                <>
                  ...
                  <button onClick={() => { setEditId(u.id); setEditName(u.name); }} title="修改单位"
                    className="w-6 h-6 rounded-md flex items-center justify-center text-[var(--tx-3)] hover:text-[var(--acc)] hover:bg-[var(--sel)] transition">
...
export function PersonAddModal({ units, nextId, onClose, onAdd }: {
  ...
  const submit = () => {
    ...
    onAdd(makePerson({
      ...
      join: `${joinYear}年7月`, startYear: joinYear, isPre2006, dutyLabel: form.position,
    }));
  };
